...
describe("promptExtensions.extractPromptExtensions", () => {
  ...
  it("非法类型兜底为空串", () => {
    expect(extractPromptExtensions({ [PROMPT_EXTENSIONS_KEY]: "not-an-object" })).toEqual({
      planner: "",
      evaluator: "",
    });
    expect(extractPromptExtensions({ [PROMPT_EXTENSIONS_KEY]: ["x"] })).toEqual({
      planner: "",
      evaluator: "",
    });
    expect(extractPromptExtensions({ [PROMPT_EXTENSIONS_KEY]: { planner: 123 } })).toEqual({
      planner: "",
      evaluator: "",
    });
  });
