...
const DEFAULT_REQUEST_TIMEOUT_MS = Number(import.meta.env.VITE_ARGUS_API_TIMEOUT_MS ?? 180000);
...
export async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    if (requestSignal.timedOut()) {
      throw new ApiError(`请求 Argus API 超时，请稍后重试：${path}`, 0, "REQUEST_TIMEOUT", {
        path,
