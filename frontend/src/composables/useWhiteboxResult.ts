...
import { getAnalysisRunSummary, listAnalysisRuns, type AnalysisRunSummary } from "../api/task";
import { errorMessage } from "../utils";

export function useWhiteboxResult(taskId: Ref<string | null>, analysisId: Ref<string | null>) {
  ...
  async function loadRun(): Promise<void> {
    ...
    loading.value = true;
    try {
      const result = await getAnalysisRunSummary(taskId.value, analysisId.value);
