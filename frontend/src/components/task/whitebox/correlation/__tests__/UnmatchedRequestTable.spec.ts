...
function makeItem(overrides: Partial<HttpRequestEvidenceInfo> = {}): HttpRequestEvidenceInfo {
  return {
...
describe("UnmatchedRequestTable", () => {
  ...
  it("多条数据时表格正常渲染", () => {
    const wrapper = mount(UnmatchedRequestTable, {
      props: {
        items: [makeItem({ requestEvidenceId: "req-a" }), makeItem({ requestEvidenceId: "req-b" })],
        total: 2,
        hasMore: false,
        loading: false,
      },
