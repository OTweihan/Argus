...
- **任务详情** — 页签布局取决于任务类型：
  ...
  **白盒任务** — 「分析报告」页签展示分析结果（概览指标、端点列表、交互式调用图查看器、发现项、执行流、功能聚类），并带有完整度横幅和分析器诊断面板；「分析日志」页签显示构建/分析进度。白盒任务不调用 LLM，因此没有 LLM 调试页签。
  ...
  当任务存在关联的黑白盒关联运行时，会显示「关联证据」页签，包含端点证据、发现证据和未匹配 HTTP 请求（见[黑白盒关联](#黑白盒关联)）。
...
- Java Analyzer 服务可达 — 默认 `http://localhost:8081`，通过环境变量 `ARGUS_JAVA_ANALYZER_URL` 覆盖。Docker Compose 部署时使用 `--profile java` 启动。
- 源码路径必须对分析进程可见且位于允许的源码根目录内（容器部署共享源码卷，见[部署指南](deployment.zh.md)）。
...
分析范围：`all`（默认，完整分析）、`changed`（增量变更）、`modules`（指定 Maven 模块）、`endpoints`、`callgraph`、`flows`、`clusters`。全部选项见 [CLI 参考](cli.zh.md#argus-analyze--执行白盒分析任务)。
...
1. 为黑盒运行创建关联运行，并固定期望的源码快照和分析配置
2. 绑定白盒分析后，捕获的 HTTP 请求与抽取的端点进行匹配
3. 匹配结果保存为端点证据和发现证据，未匹配的请求也会显式列出

### 查看关联结果

- **Web 控制台** — 存在关联运行的任务会显示「关联证据」页签，包含端点证据、发现证据和未匹配请求表格
- **REST API** — `GET /argus/api/correlation-runs/{id}`（另有 `summary`、`attempts` 及证据端点）；通过 `bind-analysis`、`retry`、`recalculate` 绑定或重新匹配
