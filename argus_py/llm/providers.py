...
PROVIDER_SPECS: dict[ModelProvider, ProviderSpec] = {
    ModelProvider.DASHSCOPE: ProviderSpec(
    ...
    ModelProvider.CUSTOM: ProviderSpec(
...
def get_provider_spec(provider: ModelProvider) -> ProviderSpec:
    """获取供应商规格。"""
    return PROVIDER_SPECS[provider]


def default_base_url(provider: ModelProvider) -> str:
    """获取供应商默认 base_url。"""
...
def create_llm_client(config: ModelConfig) -> LLMClient:
    ...
    if not spec.openai_compatible:
        raise ModelConfigError(f"供应商暂未接入 OpenAI 兼容调用：{config.provider.value}")
