...
async def navigate(
    ...
    """函数式页面打开封装，兼容简单调用。"""
    return await BrowserActions(page, navigation_timeout_ms=timeout_ms).navigate(url)


async def click(
    page: Page,
    target: str | SelectorQuery,
    timeout_ms: int = DEFAULT_ACTION_TIMEOUT_MS,
) -> dict[str, Any]:
    """函数式点击封装。"""
    return await BrowserActions(page, action_timeout_ms=timeout_ms).click(target)
...
async def fill(
    ...
    """函数式输入封装。"""
    return await BrowserActions(page, action_timeout_ms=timeout_ms).fill(target, text)


async def screenshot(page: Page, path: str | Path, full_page: bool = True) -> Path:
    """函数式截图封装。"""
    target = Path(path)
    actions = BrowserActions(page, screenshot_dir=target.parent)
    return await actions.screenshot(target.name, full_page=full_page)


async def current_state(page: Page) -> dict[str, Any]:
    """函数式页面状态封装。"""
    return await BrowserActions(page).current_state()
