...
class SelectorQuery:
    ...
    def parse(cls, raw: str) -> "SelectorQuery":
        ...
        if raw.startswith("text="):
            return cls(value=raw.removeprefix("text="), strategy="text")
        if raw.startswith("role="):
            role_part = raw.removeprefix("role=")
        ...
        if raw.startswith("label="):
            return cls(value=raw.removeprefix("label="), strategy="label")
        if raw.startswith("placeholder="):
            return cls(value=raw.removeprefix("placeholder="), strategy="placeholder")
        if raw.startswith("testid="):
            return cls(value=raw.removeprefix("testid="), strategy="test_id")
        if raw.startswith("xpath="):
            return cls(value=raw.removeprefix("xpath="), strategy="xpath")
        return cls(value=raw, strategy="css")
