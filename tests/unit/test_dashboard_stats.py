...
def test_dashboard_total_matches_full_table_count(tmp_path) -> None:
    ...
    for i in range(7):
        app._lifecycle.create_task(goal=f"goal {i}", task_type=TaskType.BLACKBOX)
...
def test_dashboard_running_total_independent(tmp_path) -> None:
    ...
    app = _make_stack(tmp_path)
    tasks = [
        app._lifecycle.create_task(goal=f"g{i}", task_type=TaskType.BLACKBOX) for i in range(3)
    ]
    app._lifecycle.update_status(tasks[0], TaskStatus.RUNNING)
